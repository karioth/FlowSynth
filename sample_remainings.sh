#!/bin/bash
#SBATCH -p gpu
#SBATCH --nodes=1
#SBATCH --gpus-per-node=H100.80gb:2
#SBATCH --ntasks-per-node=1
#SBATCH --cpus-per-task=8
#SBATCH --mem=150G
#SBATCH --time=12:00:00
#SBATCH --job-name=stable_audio_sample
#SBATCH -o %x_%j.out
#SBATCH -e %x_%j.err

set -e

NUM_GPUS=2
BATCH_SIZE=8
STEPS=100
SEED=0
PRECISION=bf16-mixed

PROMPT_CSV="/share/users/student/f/friverossego/FlowSynth/audiocaps-test.csv"
OUT_ROOT="/share/users/student/f/friverossego/FlowSynth/samples_audio_final_per_model"

FLOWSYNTH_ROOT="/share/users/student/f/friverossego/FlowSynth"

SCRATCH_BASE="/share/users/student/f/friverossego/tmp"
export TMPDIR="${SLURM_TMPDIR:-$SCRATCH_BASE/tmp}"
export TORCHINDUCTOR_CACHE_DIR="$SCRATCH_BASE/torchinductor"
export TRITON_CACHE_DIR="$SCRATCH_BASE/triton"
export CUDA_CACHE_PATH="$SCRATCH_BASE/cuda"
export XDG_CACHE_HOME="$SCRATCH_BASE/xdg"
export HF_HOME="/share/users/student/f/friverossego/.cache/huggingface"
export HUGGINGFACE_HUB_CACHE="$HF_HOME/hub"
export TRANSFORMERS_CACHE="$HUGGINGFACE_HUB_CACHE"
mkdir -p "$TMPDIR" "$TORCHINDUCTOR_CACHE_DIR" "$TRITON_CACHE_DIR" "$CUDA_CACHE_PATH" "$XDG_CACHE_HOME"
mkdir -p "$HUGGINGFACE_HUB_CACHE" "$TRANSFORMERS_CACHE"

export OMP_NUM_THREADS=1
export MKL_NUM_THREADS=1
export PYTHONUNBUFFERED=1

module purge 2>/dev/null || true
spack load miniconda3
source "$(conda info --base)/etc/profile.d/conda.sh"
conda activate jamendo

cd "$FLOWSYNTH_ROOT"

MASTER_PORT=$((10000 + SLURM_JOB_ID % 50000))

torchrun \
  --nproc_per_node="$NUM_GPUS" \
  --master_port="$((MASTER_PORT + 1))" \
  sample.py \
  --checkpoint "/share/users/student/f/friverossego/FlowSynth/audio_logs/AUDIO_NTP_AR_DiT_B_125e/checkpoints/last.ckpt" \
  --prompt-csv "/share/users/student/f/friverossego/FlowSynth/audiocaps-test.csv" \
  --cfg-scale "1" \
  --num-inference-steps "$STEPS" \
  --batch-size 64 \
  --precision "$PRECISION" \
  --seed "$SEED" \
  --output-dir "/share/users/student/f/friverossego/FlowSynth/samples_audio_final_per_model/flowsynth/AUDIO_NTP_AR_DiT_B_125e_ardiff1_vpred/cfg1"

torchrun \
  --nproc_per_node="$NUM_GPUS" \
  --master_port="$((MASTER_PORT + 1))" \
  sample.py \
  --checkpoint "/share/users/student/f/friverossego/FlowSynth/audio_logs/AUDIO_NTP_AR_DiT_B_125e/checkpoints/last.ckpt" \
  --prompt-csv "/share/users/student/f/friverossego/FlowSynth/audiocaps-test.csv" \
  --cfg-scale 3.0 \
  --num-inference-steps "$STEPS" \
  --batch-size 64 \
  --precision "$PRECISION" \
  --seed "$SEED" \
  --output-dir "/share/users/student/f/friverossego/FlowSynth/samples_audio_final_per_model/flowsynth/AUDIO_NTP_AR_DiT_B_125e_ardiff1_vpred/cfg3"

torchrun \
  --nproc_per_node="$NUM_GPUS" \
  --master_port="$((MASTER_PORT + 1))" \
  sample_stable_audio.py \
    --prompt-csv "$PROMPT_CSV" \
    --output-dir "$OUT_ROOT/stable_audio_open/cfg3" \
    --steps "$STEPS" \
    --audio-length 10 \
    --guidance-scale 3.0 \
    --negative-prompt "" \
    --batch-size "$BATCH_SIZE" \
    --seed "$SEED" \
    --precision "$PRECISION"
