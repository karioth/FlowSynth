...
#SBATCH --job-name=flowsynth_sample_all
#SBATCH -o %x_%j.out
#SBATCH -e %x_%j.err
...
AR_DIT_CKPT="/share/users/student/f/friverossego/FlowSynth/audio_logs/AUDIO_NTP_AR_DiT_B_125e/checkpoints/last.ckpt"
...
for cfg in 1 3; do
    run_flowsynth "AUDIO_NTP_AR_DiT_B_125e" "$AR_DIT_CKPT" "$cfg"
done
for cfg in 1 3; do
    run_flowsynth "AUDIO_NTP_DiT_B_sharedtime_125e" "$DIT_CKPT" "$cfg"
done
for cfg in 1 3; do
    run_flowsynth "AUDIO_NTP_MaskedAR_B_smallerhead" "$MASKEDAR_CKPT" "$cfg"
done
for cfg in 1 3; do
